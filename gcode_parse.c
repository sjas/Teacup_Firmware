...
void gcode_parse_char(uint8_t c, ParserType ctx) {
  ...
  sim_gcode_ch(c,parser);
	...
	if (last_field[ctx]) {
		...
		if ((c >= 'A' && c <= 'Z') || c == '*' || (c == 10) || (c == 13)) {
            ...
            cmd->target.axis[E] = decfloat_to_int(&read_digit[ctx], 1000);
					if (DEBUG_ECHO && (debug_flags & DEBUG_ECHO))
            serwrite_uint32(cmd->target.axis[E]);
					break;
				case 'F':
	...
	// skip comments
  if (cmd->seen_semi_comment == 0 && cmd->seen_parens_comment == 0
    #ifdef SD
    ...
    if (c >= '0' && c <= '9') {
      ...
      switch (c) {
        ...
        // comments
        case ';':
          cmd->seen_semi_comment = 1;
          break;
        case '(':
          cmd->seen_parens_comment = 1;
          break;
...
// 			request_resend();
      ...
      cmd->checksum_read = cmd->checksum_calculated = 0;
